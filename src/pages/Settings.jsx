...
import { getAuth, onAuthStateChanged, signOut } from "firebase/auth";
import { useNavigate } from "react-router-dom";
import RentalInfo from "../components/RentalInfo";  // Импортируем новый компонент
...
const Settings = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [rental, setRental] = useState(null);  // Для хранения информации об аренде
  ...
  useEffect(() => {
    // Пример загрузки информации об аренде. В реальной ситуации нужно запросить данные с сервера
    if (user) {
      // Здесь замените на фактический запрос аренды
      setRental({
  ...
  const handleCancelRental = () => {
    ...
    setRental(null);  // Пример: отмена аренды путем сброса состояния
    alert("Your rental has been canceled.");
  };
        ...
        <div className="w-2/3 p-6 flex flex-col justify-center items-center text-center">
          ...
          {user ? (
            <>
              ...
              {/* Используем компонент RentalInfo */}
              <RentalInfo rental={rental} onCancel={handleCancelRental} />
